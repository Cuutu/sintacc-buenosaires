...
describe("city/category page structure — sin FAQs duplicadas", () => {
  ...
  it("página ciudad: un bloque FAQ + SEOTextBlock sin FAQ", () => {
    ...
    expect(src).toContain("getCityH1")
    expect(src).toMatch(/<h1[\s\S]*?\{getCityH1\(city, stats\)\}/)
    expect(src).toContain("/sin-gluten/${city.slug}/${catSlug}")
    expect(src).toContain("getCityCategoryNavLabel")
    expect(src).toContain("restaurantes")
    expect(src).toContain("panaderias")
    ...
    expect(src).toContain("${BASE_URL}/sin-gluten/${ciudadSlug}")
    expect(src).not.toMatch(/mapa y recomendaciones/)
    expect(src).not.toMatch(/if\s*\(\s*ciudadSlug\s*===\s*"la-plata"[\s\S]*robots/)
    expect(src).not.toContain("/sin-gluten/la-plata/panaderías")
    expect(src).not.toContain("/sin-gluten/la-plata/cafés")
  })
