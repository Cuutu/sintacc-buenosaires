...
describe("admin centro de operaciones", () => {
  it("sidebar fija con secciones y contadores", () => {
    ...
    expect(nav).toContain("Emprendimientos")
    expect(nav).toContain("reviewsPending")
    expect(shell).toContain("Navegación móvil del admin")
    expect(shell).toContain("Más herramientas")
  ...
  it("dashboard es inbox, no grilla de botones", () => {
    ...
    expect(dash).not.toContain("Necesita atención")
    expect(dash).not.toContain("Qué debería resolver primero")
    expect(dash).toContain("Mensajes pendientes")
    expect(dash).toContain("Reseñas pendientes")
    expect(dash).toContain("Ver lugares")
    expect(dash).toContain("Completar lugares")
  ...
  it("lugares es CRM con completitud y modal por tabs", () => {
    ...
    expect(places).toContain("placeAddressLine")
    expect(places).toContain("formatShortPlaceAddress")
    expect(modal).toContain("General")
    expect(modal).toContain("Ubicación")
    expect(modal).toContain("addressText")
